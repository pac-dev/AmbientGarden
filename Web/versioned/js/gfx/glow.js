...
const flareMap = new THREE.TextureLoader().load(window.agVersionedPath+'img/glow.png');
const waveMap = new THREE.TextureLoader().load(window.agVersionedPath+'img/wave512.png');
...
events.on('timestep', () => {
	for (let rec of activeGlows) {
		...
		const deathAmt = fade(getMeta(rec).patch?.lastAmp ?? 0, 400);
		...
		flare.material.opacity = ageFactor * Math.sqrt(getMeta(rec).patch?.lastAmp ?? 0) * 0.5;
	}
});
