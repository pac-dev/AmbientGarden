...
import { getMeta, patchHush } from './beacons/beaconPool.js';
import { popUi } from './ui.js';
import { runMode } from './runMode.js';
import { beaconRecords, parsePatch } from './beacons/beaconRecords.js';

/** @param {import('./beacons/beaconRecords.js').BeaconRecord} beacon */
const getBeaconFreqs = beacon => {
	const p = beacon.patchParams;
	switch (beacon.patchName) {
		case 'vibraphones':
		case 'sine-drone':
		case 'melodic-bottle':
		...
		default:
			throw new Error('what is ' + beacon.patchName);
	}
};
		...
		*generate() {
			...
			for (let beacon1 of loadedBeacons) {
				...
				for (let beacon2 of loadedBeacons) {
					...
					if (distance > patchHush * 1.5) continue;
		...
		add(res) {
			...
			const normDist = res.distance / (patchHush * 1.2);
		...
		*generate(camX, camZ) {
		...
		add(res) {
			...
			if (res.beacon) {
				...
				res.domEle.onclick = () => {
					console.log(
						rounded(res.beacon.record.patchParams),
