...
export const playTable = ({seq, graph, table, tableVars}) => {
    ...
    const rows = table.split('\n').map(
        line => [...line.matchAll(/([^\s|_~]+)/g)].map(
            match => ({str: match[0], col: match.index, line})
        )
    ...
    const parseStepRow = row => {
        ...
        const colTimes = {};
        let timeStepper = 0, prevInterval = 0;
    ...
    const parseAutoRow = row => {
        ...
        const name1 = match[3], name2 = match[4];
        let param;
        ...
        for (let [i, token] of row.bodyTokens.entries()) {
            ...
            token.event.actions.push(
                () => seq.ctrlSlide({param, endVal, dur, type})
            );
            lastValue = endVal;
        }
    };
    ...
    const rowSignatures = [
        ['step', parseStepRow], ['markers', parseMarkersRow],
        ['(', parseAutoRow], ['fn', parseFnRow]
    ];
    for (let row of rows) {
